// ============================================================================
// Swarm YAML Schema — адаптировано из oh-my-pi/packages/swarm-extension
//
// Формат YAML:
//   swarm:
//     name: feature-impl
//     workspace: .          # путь относительно YAML-файла
//     mode: pipeline          # pipeline | parallel | sequential
//     target_count: 1         # кол-во итераций (только для pipeline)
//     model: deepseek-v4-pro  # модель по умолчанию для всех агентов
//     agents:
//       planner:
//         role: Архитектор
//         task: Спланируй реализацию фичи X
//         model: deepseek-v4-pro  # переопределение для конкретного агента
//         waits_for: []           # зависимости
//         reports_to: []          # кому отправлять результат
// ============================================================================
...
export function parseSwarmYaml(content: string): SwarmDefinition {
	...
	const raw = yamlParse(content) as { swarm?: RawSwarmConfig } | null;
	if (!raw?.swarm) {
