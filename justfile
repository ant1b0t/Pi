...
ext-pure-focus:
    pi -e extensions/examples/pure-focus.ts

# 3. Minimal pi: model name + 10-block context meter
ext-minimal:
    pi -e extensions/examples/minimal.ts -e extensions/examples/theme-cycler.ts

# 4. Cross-agent pi: load commands from .claude/, .gemini/, .codex/ dirs
ext-cross-agent:
    pi -e extensions/examples/cross-agent.ts -e extensions/examples/minimal.ts
...
ext-purpose-gate:
    pi -e extensions/examples/purpose-gate.ts -e extensions/examples/minimal.ts

# 6. Customized footer pi: Tool counter, model, branch, cwd, cost, etc.
ext-tool-counter:
    pi -e extensions/examples/tool-counter.ts

# 7. Tool counter widget: tool call counts in a below-editor widget
ext-tool-counter-widget:
    pi -e extensions/examples/tool-counter-widget.ts -e extensions/examples/minimal.ts

# 8. Subagent widget: /sub <task> with live streaming progress
ext-subagent-widget:
    pi -e extensions/examples/subagent-widget.ts -e extensions/examples/pure-focus.ts -e extensions/examples/theme-cycler.ts

# 9. TillDone: task-driven discipline — define tasks before working
ext-tilldone:
    pi -e extensions/examples/tilldone.ts -e extensions/examples/theme-cycler.ts

#g2

# 10. Agent team: dispatcher orchestrator with team select and grid dashboard
ext-agent-team:
    pi -e extensions/examples/agent-team.ts -e extensions/examples/theme-cycler.ts

# 11. System select: /system to pick an agent persona as system prompt
ext-system-select:
    pi -e extensions/examples/system-select.ts -e extensions/examples/minimal.ts -e extensions/examples/theme-cycler.ts

# 12. Launch with Damage-Control safety auditing
ext-damage-control:
    pi -e extensions/examples/damage-control.ts -e extensions/examples/minimal.ts -e extensions/examples/theme-cycler.ts

# 13. Agent chain: sequential pipeline orchestrator
ext-agent-chain:
    pi -e extensions/examples/agent-chain.ts -e extensions/examples/theme-cycler.ts

#g3

# 14. Pi Pi: meta-agent that builds Pi agents with parallel expert research
ext-pi-pi:
    pi -e extensions/examples/pi-pi.ts -e extensions/examples/theme-cycler.ts

#ext

# 15. Session Replay: scrollable timeline overlay of session history (legit)
ext-session-replay:
    pi -e extensions/examples/session-replay.ts -e extensions/examples/minimal.ts
...
# 19. Base Agents with base-tools: includes web_fetch, todo, ask_user, glob
ext-base-agents-full:
    pi -e extensions/base/base-agents.ts -e extensions/base/base-tools.ts

# 20. Agent Team with base-agents (dispatcher orchestrator)
ext-agent-team:
    pi -e extensions/base/base-agents.ts -e extensions/examples/agent-team.ts -e extensions/examples/theme-cycler.ts

# 21. Agent Chain with base-agents (sequential pipeline)
ext-agent-chain:
    pi -e extensions/base/base-agents.ts -e extensions/examples/agent-chain.ts -e extensions/examples/theme-cycler.ts
